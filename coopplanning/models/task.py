...
class TaskTemplate(models.Model):
    ...
    duration = fields.Float(
        compute="_compute_duration", help="Duration in Hour", store=True
    )
            ...
            task.create(
                {
                    ...
                    % (
                        self.name,
                        float_to_time(self.start_time),
                        float_to_time(self.end_time),
                        i,
                    ),
                    "task_template_id": self.id,
                    "task_type_id": self.task_type_id.id,
                    "worker_id": self.worker_ids[i].id
                    if len(self.worker_ids) > i
                    else False,
                    "start_time": fields.Datetime.today().replace(
                        hour=h_begin, minute=m_begin, second=0
                    ),
                    "end_time": fields.Datetime.today().replace(
                        hour=h_end, minute=m_end, second=0
                    ),
                }
            )
